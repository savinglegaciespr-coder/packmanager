...
async def purge_collections(admin: Dict[str, Any] = Depends(require_role("superadmin"))) -> Dict[str, Any]:
    r_bookings = await db.bookings.delete_many({})
    r_email_logs = await db.email_logs.delete_many({})
    return {
        "bookings_deleted": r_bookings.deleted_count,
        "email_logs_deleted": r_email_logs.deleted_count,
    }
